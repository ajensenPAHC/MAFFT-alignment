...
def sanitize_id(base_id, seen):
    safe_id = re.sub(r'[^A-Za-z0-9_]', '_', base_id)[:20]
    orig_id = safe_id
    counter = 1
    ...
    if st.button("Submit Sequences for Alignment"):
        ...
        for i in selected_rows:
            ...
            if cleaned_seq and len(cleaned_seq) >= 10:
                ...
                safe_id = sanitize_id(f"row{i}_{name}", seen_ids)
        ...
        if use_uploaded_ref:
            ...
            ref_record = SeqRecord(Seq(sequences[ref_idx]), id=names[ref_idx], description="")

        all_records = [ref_record] + [SeqRecord(Seq(seq), id=name, description="") for name, seq in zip(names, sequences) if name != ref_record.id]
        ...
        if compute_individual_alignments:
            ...
            for i, record in enumerate(all_records):
                ...
                progress.progress((i + 1) / len(all_records))
                styled_table = df_results.style.map(color_identity, subset=[col for col in df_results.columns if "%" in col])
                placeholder_table.dataframe(styled_table)
